...
export function getPlatformId (): string {
    return window.cordova && window.cordova.platformId || getBrowserInfo(true).toLowerCase()
}

export function getPlatform () {
    return isElectron() ? 'Electron' : (window.device && window.device.platform) || 'Browser'
}
